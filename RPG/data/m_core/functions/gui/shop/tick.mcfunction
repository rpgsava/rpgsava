...
#クリックを検知
execute store result score @s Core.Shop.IsClickItem run clear @a[tag=Shop.Practitioner,limit=1] #m_core:shop_item{ShopItem:true}
...
execute if score $Return Core.System.Input matches -2 run data modify entity @s Items append value {Slot:16b,Count:1b,id:"minecraft:red_stained_glass",tag:{ShopItem:true,display:{Name:'[{"translate":"RPG.Shop.NoSpace"}]'}}}
