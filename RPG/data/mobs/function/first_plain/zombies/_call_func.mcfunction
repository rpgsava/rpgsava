...
function mobs:_call
data merge entity @s {IsBaby:False,CustomNameVisible:True,Tags:["firstPlainsZombie","sectionFirstEnemy","firstPlainEnemy","Enemy"]}
data modify entity @s CustomName set from storage mobs:entity CustomName

#防具の決定(防御力は変数により決定されるので防具は見た目のみ)
data merge entity @s {ArmorItems:[{},{},{},{components: {"minecraft:unbreakable": {show_in_tooltip: 0b}}, count: 1, id: "minecraft:leather_helmet"}]}
